...
★★ 2026-08-12 起這支**不適用於目前的掛法,不要用它的建議值** ★★

Mid-360 的原生垂直視野是 -7 ~ +52 度(本來就是往上看的),再加上機構
往上斜 29.69 度,實測掃到的點裡:

    z = -0.10 ~ +0.05 m(地面)          8 點    0.0%
    z = +2.00 ~ +4.00 m(天花板)   390324 點   45.8%

**地面點只有 8 個。** 沒有地面可以擬合,RANSAC 只好去挑牆或天花板,
實測回報「內點 20%、法線離垂直 9.05 度、建議 BOX_Z 改成 -0.3223」——
全部是垃圾。輸出訊息裡的 0.2510 也是寫死的舊值,不會讀現在的 BOX_Z。

現在 BOX_Z / BOX_X 改用捲尺直接量(見 robot_tf.sh 的註解),
量單一段、沒有誤差轉嫁,比擬合可靠。這支留著只當歷史記錄。
