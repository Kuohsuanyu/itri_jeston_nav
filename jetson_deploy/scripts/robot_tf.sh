...
#
#   盒底離地              0.5300   ← 捲尺
#   base_link 離地        0.2032   ← 輪半徑,原廠 URDF chassis_DD-M.xacro
#                         ──────
#   BOX_Z                 0.3268
#
#   盒中心往前            0.1300   ← 捲尺,base_link 原點在底盤幾何中心
#                                    (base_link.STL 的 X 是 ±0.5049,對稱)
#
# ── 為什麼盒子是懸空的 ──────────────────────────────────────────
#   base_link.STL 頂面    0.2490   (4048 個頂點的大平面 = 上蓋)
#   BOX_Z                 0.3268
#                         ──────
#   盒底離上蓋            0.0778   支架的高度。盒子沒有直接坐在上蓋上。
#
# RViz 裡看起來會浮著,那是對的 —— 模型沒有畫支架而已。
#
# ── 和先前地面擬合值的差異 ──────────────────────────────────────
...
#
# ★ 這個值錯的後果:start_slam2d.sh 的 /scan 高度帶會整段平移,
