...
# ---- box_link 相對各來源的固定變換 --------------------------------------
# ★ 跟 robot_tf.sh 用同一組數字,改那邊就要改這邊。
# BOX_Z 0.3705 是 calib_height.py 實測(2026-08-11),不是機構圖的 0.2510。
BOX_X, BOX_Y, BOX_Z, BOX_YAW = 0.0000, 0.0000, 0.3705, 0.0000   # base_link → box_link
AXLE_Z = 0.2032                                                  # base_footprint → base_link
# box_link → body(calib_box.py 實測)
BODY = (0.0250, -0.0000, 0.2005, 0.0112, 0.5181, 0.0000)
...
class Relay(Node):
    ...
    def _emit(self, src, T_src_bf, pub, sxy, syaw):
        ...
        # twist 兩邊都是 0,不轉發 —— EKF 設定裡也不會去讀它
        out.twist.covariance = cov6(1e3, 1e3)
        pub.publish(out)

    def on_wheel(self, msg):
        self.n_w += 1
        self._emit(msg, T_IDENT, self.pub_w, WHEEL_SIGMA_XY, WHEEL_SIGMA_YAW)

    def on_lidar(self, msg):
        self.n_l += 1
        self._emit(msg, T_BODY_BF, self.pub_l, LIDAR_SIGMA_XY, LIDAR_SIGMA_YAW)
